...
- **키는 `NAVER_MAP_KEY_ID` 환경변수로 빌드 때 들어온다**(`src/_data/env.js`).
  비밀값이라서가 아니라 — 스크립트 주소에 붙어 그대로 노출된다 — **저장소가
  공개**라 커밋 이력에 남기지 않으려는 것이다. 실제 방어는 네이버 콘솔의
  **Web 서비스 URL 제한**이고, 지도가 안 뜨면 **거기부터 본다.**
